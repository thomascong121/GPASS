...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        print('Level dimensions: ', self.level_dim)
        ...
        # print('Processing WSI: ', path)
        # if os.path.exists(f'/g/data/iq24/CAMELYON16/train/lesion_annotations/{self.name}.xml'):
        #     self.initXML(f'/g/data/iq24/CAMELYON16/train/lesion_annotations/{self.name}.xml')
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        if self.contours_tumor is not None:
            n_contour_tumor = len(self.contours_tumor)
            print("Total number of tumor contours to process: ", n_contour_tumor)
        fp_chunk_size = math.ceil(n_contours * 0.05)
        init = True
        for idx, cont in enumerate(self.contours_tissue):
            if (idx + 1) % fp_chunk_size == fp_chunk_size:
                print('Processing contour {}/{}'.format(idx, n_contours))
            asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], patch_level, save_path, self.contours_tumor, patch_size, step_size, **kwargs)
            if len(asset_dict) > 0:
                if init:
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, cont_tumor=None, patch_size = 256, step_size = 256,
        ...
        if cont_tumor is not None:
            iterable = [(coord, contour_holes, ref_patch_size[0], cont_check_fn, cont_tumor) for coord in coord_candidates]
            labels = pool.starmap(WholeSlideImage.process_coord_candidate_label, iterable)
        pool.close()
    ...
    @staticmethod
    def process_coord_candidate_label(coord, contour_holes, ref_patch_size, cont_check_fn, cont_tumor):
        if WholeSlideImage.isInContours(cont_check_fn, coord, contour_holes, ref_patch_size):
            for cont in cont_tumor:
                if isinstance(cont_check_fn, str):
                    if cont_check_fn == 'four_pt':
                        cont_tum_check_fn = isInContourV3_Easy(contour=cont, patch_size=ref_patch_size[0],
                                                               center_shift=0.5)
                    elif cont_check_fn == 'four_pt_hard':
                        cont_tum_check_fn = isInContourV3_Hard(contour=cont, patch_size=ref_patch_size[0],
                                                               center_shift=0.5)
                    elif cont_check_fn == 'center':
                        cont_tum_check_fn = isInContourV2(contour=cont, patch_size=ref_patch_size[0])
                    elif cont_check_fn == 'basic':
                        cont_tum_check_fn = isInContourV1(contour=cont)
                    else:
                        raise NotImplementedError
                else:
                    assert isinstance(cont_check_fn, Contour_Checking_fn)
                    cont_tum_check_fn = cont_check_fn
                if WholeSlideImage.isInContours(cont_tum_check_fn, coord, contour_holes, ref_patch_size):
                    return 1
                else:
                    return 0
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        count = 0
        print('\naccumulating attention scores')
        for idx in range(len(coords)):
            score = scores[idx]
            coord = coords[idx]
            # swape the min and max of the score
            # score = 1 - score
            if score >= threshold:
                if binarize:
                    score=1.0
