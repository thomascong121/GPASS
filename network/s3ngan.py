...
class twodecoderGANModel(nn.Module):
    ...
    def test_single_patch(self, input, method='avg'):
        self.netG.eval()
        with torch.no_grad():
            image_gray, _ = image_read(self.opt.dataset.opt_dataset,
                                    input,
                                    augment_fn=self.opt.dataset.opt_dataset['augment_fn'],
                                    img_index=self.opt.dataset.opt_dataset['image_index']
                                    )
            prediction1, prediction2 = self.netG(image_gray.to(self.gpu_ids[0]).float().unsqueeze(0))
            if method == 'avg':
    ...
    def test(self, test_loader, stage='test', save=False, source_slide_name=None, logger=None):
        """
        ...
        out_img_pth = os.path.join(test_root, str(self.opt.run.opt_run['which_epoch']), stage, source_slide_name)
        out_img_pth = f'{out_img_pth}/patches'
        ...
        with torch.no_grad():
            ...
            for i, (input, target) in tqdm(enumerate(test_loader)):
                ...
                prediction = prediction2
                # get nmi for real image
                nmi_base_patch = normalized_median_intensity_95(target, logger)
                # get nmi for predicted image
                nmi_pred_patch = normalized_median_intensity_95(prediction, logger)
                if logger is not None:
                    logger.debug(f'NMI_base: {nmi_base_patch}, NMI_pred: {nmi_pred_patch}')
                # Clip values to ensure they're within [0,1] range
                prediction = torch.clamp(prediction, 0.0, 1.0)
                target = torch.clamp(target, 0.0, 1.0)
                
                # Calculate PSNR and SSIM
                psnr_val = psnr(prediction, target, convert_to_greyscale=True)
                ssim_val = ssim(prediction, target, data_range=1.0)
                
                avg_psnr += psnr_val
                avg_ssim += ssim_val
                nmi_base += nmi_base_patch
                nmi_pred += nmi_pred_patch
                steps += 1
                 # Save images if requested
                if save:
                    # Save the image
                    save_dir = self.opt.run.opt_run['checkpoints_dir'] + '/' + \
                                  self.opt.dataset.opt_dataset['name'] + '/' + \
                                  'normalised/' + str(self.opt.run.opt_run['which_epoch']) + '/' + \
                                  stage + '/' + \
                                  source_slide_name + '/patches'
                    os.makedirs(save_dir, exist_ok=True)
                    
                    # Save the image
                    image_1 = tensor2im(prediction1[0])
                    image_1 = Image.fromarray(image_1)
                    image_2 = tensor2im(prediction2[0])
                    image_2 = Image.fromarray(image_2)
                    
                    # Save two images target+prediction in a single plot
                    target_img = tensor2im(target[0])
                    target_img = Image.fromarray(target_img)

                    input_img = tensor2im(input[0], denormalize=True)
                    input_img = Image.fromarray(input_img[:, :, 0])

                    fig, axes = plt.subplots(1, 4, figsize=(20, 5))
                    for ax, img in zip(axes, (input_img, target_img, image_1, image_2)):
                        # save the first image as grayscale
                        if ax == axes[0]:
                            ax.imshow(img, cmap='gray')
                        else:
                            ax.imshow(img)
                        ax.axis("off")
                    fig.savefig(os.path.join(save_dir, f'patch_{i}.png'), dpi=300, bbox_inches="tight")
                    plt.close(fig)

        self.netG.train()
        # Return statistics instead of raw values
        return avg_psnr / steps, avg_ssim / steps, nmi_base / steps, nmi_pred / steps
        #             # combine both images and save
        #             prediction_save = Image.new('RGB', (original.width + normalised.width, original.height))
        #             prediction_save.paste(original, (0, 0))
        #             prediction_save.paste(normalised, (original.width, 0))
