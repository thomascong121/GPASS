...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, draw_grid=True, stain_norm=False):
    ...
    for idx in tqdm(range(total)):        
        ...
        if stain_norm:
            # check if stain norm is a pytorch nn.Module
            if isinstance(stain_norm, nn.Module):
                # convert patch to PIL image
                patch = Image.fromarray(patch)
                to_gray = transforms.Grayscale(3)
                patch = transforms.ToTensor()(to_gray(patch))
                patch = transforms.Resize((256, 256))(patch)
                patch = patch.unsqueeze(0)
                # print('Size of patch: ', patch.size())
                if torch.cuda.is_available():
                    patch = patch.cuda()
                prediction1, prediction2 = stain_norm.netG(patch.float())
                patch = (prediction1 + prediction2) / 2
                patch = tensor2im(patch.squeeze(0))
            else:
                patch = patch[..., ::-1]
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1, stain_norm=False):
    ...
    heatmap = np.array(heatmap)
    heatmap = DrawMapFromCoords(heatmap, wsi_object, coords, patch_size, vis_level, indices=None, draw_grid=draw_grid, stain_norm=stain_norm)
    return heatmap
